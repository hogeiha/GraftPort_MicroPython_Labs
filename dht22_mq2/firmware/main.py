...
from drivers.dht_driver import DHTBase, DHT22
...
from tasks.sensor_task import DhtOledTask
...
mq2_DIN = board.get_adc_pins(1)[0]
mq = MQX(comp_pin=Pin(mq2_DIN, Pin.IN), user_cb=mq_callback, rl_ohm=10000, vref=3.3,irq_trigger=Pin.IRQ_FALLING)
# 选择内置多项式（MQ2、MQ4、MQ7）
mq.select_builtin("MQ2")
# DHT22引脚
dht22_pin=board.get_adc_pins(2)[0]
# 创建DHT22传感器的实例
d = DHT22(Pin(dht22_pin))
...
# 创建传感器-蜂鸣器-LED任务实例
sensor_task_obj = DhtOledTask(DHT22=d, MQX=mq, SSD1306_I2C=oled,debug=True)
