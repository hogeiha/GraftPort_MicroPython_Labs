...
from drivers.iic_bus_16_servo_driver import PCA9685, BusPWMServoController
...
pir_pin=board.get_dio_pins(0)[0]
# 创建红外人体热释传感器对象
...
# 若非空，则打印从机设备地址
    ...
    # 遍历从机设备地址列表
    for PCF8574_ADDR in devices_list:
        # 判断设备地址是否为PCF8575的地址
        if 0x40 >= PCF8574_ADDR and PCF8574_ADDR <= 0x4F:
            print("I2C hexadecimal address: ", hex(PCF8574_ADDR))
pca = PCA9685(i2c,  address=PCF8574_ADDR)
...
# 创建传感器-蜂鸣器-LED任务实例
sensor_task_obj = sensorTask(PIRSensor=pir, BusPWMServoController=srv,debug=True)
