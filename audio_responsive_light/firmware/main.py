...
from libs.TFT_mic import AudioFrequencyAnalyzer

#导入drivers文件夹下面传感器模块驱动库
from drivers.max9814_mic_driver import MAX9814Mic
...
from tasks.sensor_task import micTask
...
# 获取mic引脚
MIC_AIN,MIC_DIN = board.get_adc_pins(0)
# 创建mic实例
mic = MAX9814Mic(adc=ADC(MIC_AIN), gain_pin=Pin(MIC_DIN, Pin.OUT))
# 创建mic频率分析实例
analyzer = AudioFrequencyAnalyzer(mic)
# 获取I2C0的引脚编号
...
# 创建传感器-MIC-LED任务实例
sensor_task_obj = micTask(AudioFrequencyAnalyzer=analyzer, LEDBar=ledbar,debug=True)
