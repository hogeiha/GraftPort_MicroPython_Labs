...
from libs.oledmenu import MenuNode,SimpleOLEDMenu
# 导入drivers文件夹下面传感器模块驱动库
from drivers.pcf8574keys_driver import PCF8574Keys, PCF8574, KEYS_MAP
from drivers.ssd1306_driver import SSD1306_I2C
from drivers.si5351_driver import SI5351_I2C
...
# 晶振频率 25 MHz
crystal = 25e6
# I2C0 时钟频率 (Hz)，默认 400kHz
I2C0_FREQ = 400_000
# I2C1 时钟频率 (Hz)，默认 100kHz
I2C1_FREQ = 100_000
# OLED ssd1306的iic地址
OLED_ADDRESS = 0x3d
# iickey
...
i2c1 = I2C(id=1, sda=i2c1_sda_pin, scl=i2c1_scl_pin, freq=I2C1_FREQ)
...
# 若非空，则打印从机设备地址
    ...
    for device in devices_list:
        ...
        if device == 0x60 or device == 0x61:
            SI5351_ADDRESS = device

# 创建SI5351的实例
si5351 = SI5351_I2C(i2c=i2c1,crystal=crystal,address=SI5351_ADDRESS)
...
menu = SimpleOLEDMenu(oled, name="Main Menu", pos_x=0, pos_y=0, width=128, height=64)

pcf = PCF8574(i2c, KEYS_ADDRESS)
# 初始化五向按键
...
# 创建传感器-蜂鸣器-LED任务实例
sensor_task_obj = sensorTask(PCF8574Keys=keys, SimpleOLEDMenu=menu, SI5351_I2C=si5351, debug=True)
